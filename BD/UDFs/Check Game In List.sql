...
as
	begin
		if (not exists( select *
						...
						where GamesDB.[Events].UserName=@userName and GamesDB.Games.GameID=@gameID
							and GamesDB.[Events].TypeID = (select TypeID
															from GamesDB.EventType
															where Name=@listName and ChangeDate is null
															)
					)
			)
			return 0
